...
#
# Usage: ./machine.sh start|stop|status [btc|weather|all]
...
machines() {
  case "$TARGET" in
    ...
    all) echo "btc weather" ;;
    *) echo "Unknown machine: $TARGET (use btc|weather|all)" >&2; exit 1 ;;
  esac
}
...
for M in $(machines); do
  ...
  case "$CMD" in
    ...
    *) echo "Usage: machine.sh start|stop|status [btc|weather|all]" >&2; exit 1 ;;
  esac
done
